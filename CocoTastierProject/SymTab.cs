...
public class Obj { // properties of declared symbol
   ...
   public int constant; // check if constant
...
// close current scope
   public void CloseScope() {
      String kindToPrint = "";
      String typeToPrint = "";
      Obj printScope = topScope;
      Obj obj;
      printScope = topScope;

         obj = printScope.locals;
         while (obj != null) { 
         switch(obj.kind){
               case 0:
               if(curLevel == 0) kindToPrint = "Global Variable";
               else kindToPrint = "Local Variable";
               break;
               case 1:
               kindToPrint = "Procedure";
               typeToPrint = "Undef";
               break;
               case 2:
               kindToPrint = "Scope";
	       ...
	       break;
            ...
            Console.WriteLine(";" + obj.name + " " + ifConstant + " " + kindToPrint + " " + typeToPrint + " " + obj.adr);
...
// create new object node in current scope
   public Obj NewObj(string name, int kind, int type, int constantFlag) {
      Obj p, last; 
      Obj obj = new Obj();
      obj.name = name; obj.kind = kind;
      obj.type = type; obj.level = curLevel; 
      obj.next = null; 
      obj.constant = constantFlag;
      if(kind == proc){
         parser.SemErr("Cannot have a constant procedure");
      }
      p = topScope.locals; last = null;
      while (p != null) { 
         if (p.name == name)
